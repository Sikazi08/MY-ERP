...
export default function Clients() {
  ...
  const { data: clientDetail, isLoading: isLoadingDetail } = useGetClient(selectedClientId as number, { query: { queryKey: getGetClientQueryKey(selectedClientId as number), enabled: !!selectedClientId } });

  const handleExport = () => {
    window.open('/api/exports/clients', '_blank');
  };
              ...
              <div>
                ...
                <div className="space-y-3">
                  {clientDetail.purchases && clientDetail.purchases.length > 0 ? (
                    ...
                    <p className="text-muted-foreground">Aucun achat détaillé disponible.</p>
                  )}
                </div>
              </div>
            </div>
